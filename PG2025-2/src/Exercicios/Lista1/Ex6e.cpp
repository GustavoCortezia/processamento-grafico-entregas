...
glDrawArrays(GL_TRIANGLE_FAN, 0, NUM_POINTS * 2 + 2);
...
int setupGeometry()
{
    const float radius_outer = 0.7f; // raio externo (pontas)
    const float radius_inner = 0.3f; // raio interno (reentrâncias)
    
    std::vector<GLfloat> vertices;

    // centro da estrela (necessário para TRIANGLE_FAN)
    vertices.push_back(0.0f);
    vertices.push_back(0.0f);
    vertices.push_back(0.0f);

    for (int i = 0; i <= 5 * 2; i++) {
        float angle = i * M_PI / 5; // cada passo = 180°/N
